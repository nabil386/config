...
( function() {
	...
	CKEDITOR.plugins.add( 'ibmuploadimage', {
		requires: 'dialog',
		lang: 'af,ar,bg,bn,bs,ca,cs,cy,da,de,el,en,en-au,en-ca,en-gb,eo,es,et,eu,fa,fi,fo,fr,fr-ca,gl,gu,he,hi,hr,hu,id,is,it,ja,ka,km,ko,ku,lt,lv,mk,mn,ms,nb,nl,no,pl,pt,pt-br,ro,ru,si,sk,sl,sq,sr,sr-latn,sv,th,tr,tt,ug,uk,vi,zh,zh-cn', // %REMOVE_LINE_CORE%
		icons: 'image', // %REMOVE_LINE_CORE%
		hidpi: true, // %REMOVE_LINE_CORE%
		init: function( editor ) {
			// Abort when Image/ Image2 is to be loaded since both plugins
			// share the same button, command, etc. names (#11222).
			if ( editor.plugins.image || editor.plugins.image2 )
				return;

			var pluginName = 'ibmuploadimage';

			// Register the dialog.
			CKEDITOR.dialog.add( pluginName, this.path + 'dialogs/ibmuploadimage.js' );
			...
			editor.on( 'doubleclick', function( evt ) {
				...
				if ( element.is( 'img' ) && !element.data( 'cke-realelement' ) && !element.isReadOnly() )
					evt.data.dialog = 'ibmuploadimage';
			} );
			...
			if ( editor.addMenuItems ) {
				editor.addMenuItems( {
					ibmuploadimage: {
						label: editor.lang.ibmuploadimage.menu,
						command: 'ibmuploadimage',
                        icon: 'image',
						group: 'image'
					}
				} );
			...
			if ( editor.contextMenu ) {
				editor.contextMenu.addListener( function( element, selection ) {
					if ( getSelectedImage( editor, element ) )
						return { ibmuploadimage: CKEDITOR.TRISTATE_OFF };
				} );
			}
		},

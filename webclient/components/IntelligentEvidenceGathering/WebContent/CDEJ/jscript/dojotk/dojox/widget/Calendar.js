...
require(["dijit/_Widget", "dijit/_Templated", "dijit/_Container", "dijit/typematic", "dijit/Calendar", "dojo/date", "dojo/date/locale"]);
...
dojo.declare("dojox.widget._CalendarBase", [dijit._Widget, dijit._Templated, dijit._Container], {
	...
	postCreate: function(){
		...
		this.footer.innerHTML = this.today + ": "
	...
	startup: function(){
		dojo.subscribe("dojoxCalendarResize", function(){
			var cls = "dojoxCalResize";
			dojo.addClass(dojo.body(), cls);
			dojo.removeClass(dojo.body(), cls);
		});	
	},
	
	updateView: function(){
		dojo.publish("dojoxCalendarResize");
		
		setTimeout(function(){dojo.publish("dojoxCalendarResize");}, 50);
	},
...
dojo.declare("dojox.widget._CalendarDayView", [dojox.widget._CalendarView, dijit._Templated], {
	...
	_onDayClick: function(e){
		...
		var label = e.target;
		if (dojo.hasClass(label, "dijitCalendarDateTemplate")) {
		  label = dojo.query(".dijitCalendarDateLabel", label)[0];
		}

...
define(["dojo/_base/array",
   ...
   */
  return oneuiRoot.Header = declare("curam.app.Header", [_Widget, _TemplatedMixin], 
  /** @lends curam.app.Header.prototype */
  {
    /**
    ...
    /**
     * A dijit.Menu to be used as the popup of available actions for
     * logout. If a single dijit.MenuItem is supplied, the log out text will be
     * presented as a simple action and onClick will be triggered on
    ...
    */
    ...
    //  logoutTextNode: contains log out text
    //  
    ...
    _refreshUser: function(){
      if(this.userNode){
        ...
        domclass.replace(this.hoverNode, msg ? "idxHeaderUserName" : "idxHeaderUserNameNoText", "idxHeaderUserName idxHeaderUserNameNoText");
      }
    ...
    _setup: function(){
      ...
      var show_primary_title = this.primaryTitle,
        ...
        show_primary_logout = this.logout,
        ...
        show_tertiary_content = show_content && !show_secondary_content,
          show_primary_items = show_primary_title || show_primary_logo || show_primary_help || show_primary_settings || show_primary_sharing || show_primary_user || show_primary_navigation || show_primary_search || show_primary_logout,  
      ...
      require(modules, function(){
        ...
        if(show_primary_logout){
                    me._renderLogout(me._trailingGlobalActionsNode, show_primary_user);
                }
        if((me.user&&me.user.actions) || me.sharing || me.settings || me.help || me.logout){
    ...
    _reset: function(){
      ...
      if (this.logout) {
        this.logout = registry.byId(this.logout);
        if (this.logout) this.logout.placeAt(this.containerNode);
      }
      this._clearHandles("logoutHandles");
      ...
      this.logoutNode = null;
      this.logoutTextNode = null;
    ...
    destroy: function() {
      ...
      if (this.logout) {
        if (this.logout.destroyRecursive) this.settings.destroyRecursive();
        else if (this.logout.destroy) this.logout.destroy();
        this.logout = null;
      }
      this._clearHandles("logoutHandles");
    ...
    _setShowUserDropDownArrowAttr: function(value) {
      ...
      if (this.hoverNode) {
        domclass.toggle(this.hoverNode, "idxHeaderDropDown", this.showUserDropDownArrow);
      }
    },
    ...
    /**
     * Handler for setting the logout.  This ensures
     * that the header gets refreshed if needed or that at
     * least the header gets marked as requiring a refresh.
     *
     * @param {Widget} widget The widget to use for the logout
     * @private
     */
    _setLogout: function(value) {
      var previous = this.logout;
      this.logout = value;
      if (previous != value) {
        this._refresh();
      }
    ...
    _renderLogout: function(domNode, addSeparator){
      if(addSeparator){
        this._renderSeparator(domNode);
      }
      this._injectTemplate(domNode,
                     '<li class="idxHeaderPrimaryAction">' +
                     '<a tabindex="0" href="javascript://" data-dojo-attach-point="logoutNode" role="presentation" class="idxHeaderUserName">' +
                     '<span class="idxHeaderUserText" data-dojo-attach-point="logoutTextNode" onclick=CuramExternalApp.displayContent({"param":[],"pageID":"LogoutWrapper"});>' +
                      '</span>' +
                      '</a>' +
                     '</li>');
      ...
      if(this.logout){
        this.logout = registry.byId(this.logout);
        this.logoutTextNode.innerHTML = this.logout.message
        this._clearHandles("_actionsHandles");
        this.logoutNode.setAttribute("role", "menuitem");
        this.logoutNode.setAttribute("aria-haspopup", true);
      }
    },
    ...
    _renderUser: function(domNode){
      this._injectTemplate(domNode,
              '<li class="idxHeaderPrimaryAction">' + 
	              '<a tabindex="0" href="javascript://"  class="idxHeaderUserNameNoText" data-dojo-attach-point="userNode" role="presentation">' +
	                ...
	                '</span>' +
	                '</a>' +
	                '<a href="javascript://" class="idxHeaderUserNameNoText" role="presentation" data-dojo-attach-point="hoverNode">' +
	                    '<span class="idxHeaderDropDownArrow">' +
	                      '<span class="idxTextAlternative">(v)</span>' +
	                    '</span>' +
	              '</a>' +
      ...
      if(this.user && this.user.actions){
        this.user.actions = registry.byId(this.user.actions);
        this._clearHandles("_actionsHandles");
        this._actionsHandles = this._prepareMenu(this.user.actions, [ "oneuiHeaderGlobalActionsMenu", "oneuiHeaderGlobalActionsSubmenu" ], this.hoverNode, this.hoverNode, true);
        domclass.toggle(this.hoverNode, "idxHeaderDropDown", this.showUserDropDownArrow);
        this.hoverNode.setAttribute("role", "menuitem");
        this.hoverNode.setAttribute("aria-haspopup", true);
      }
    },

    ...
    <fo:root>
      ...
      <fo:page-sequence master-reference="Section1-ps">
        ...
        <fo:flow flow-name="xsl-region-body" font-family="WT Sans">
          ...
          <fo:block xsl:use-attribute-sets="Normal_2"> </fo:block>
          <fo:block xsl:use-attribute-sets="Normal_2">
            Vous avez reçu l'ordre de comparaître au nom de
            <xsl:apply-templates select="FIELD[FNAME='behalfOfName']" />
            aux date et heure précisées ci-dessous :
          </fo:block>
          <fo:block xsl:use-attribute-sets="Normal_2"> </fo:block>
          ...
          <fo:block xsl:use-attribute-sets="Normal_2">Votre présence est obligatoire aux date et heure indiquées sur la présente notification pour participer à l'affaire concernant la ou les personnes précitées.</fo:block>
          <fo:block xsl:use-attribute-sets="Normal_2"> </fo:block>
          <fo:block xsl:use-attribute-sets="Normal_2">Si vous avez une raison valable de ne pas comparaître à cette audience, vous devez en rendre compte auprès du responsable de l'audience. En cas de non-comparution, sachez que la cour peut intenter des actions en justice supplémentaires.</fo:block>

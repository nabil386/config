...
import curam.rules.functions.CustomFunctionPrepopulateDetails;
...
/**
 * The Class tests {@link CustomFunctionPrepopulateDetails} custom
 * function.
 */

@RunWith(JMockit.class)
public class CustomFunctionPrepopulateDetailsTest
  ...
  @Tested
  CustomFunctionPrepopulateDetails customFunction;

  public CustomFunctionPrepopulateDetailsTest() {
  ...
  public void before() throws NoSuchSchemaException {
    ...
    customFunction = new CustomFunctionPrepopulateDetails();
  ...
  public void test_valid()
    ...
    new Expectations(datastore) {

      {
        ...
        personObj.read((PersonKey) any);
        result = personDtls;

        curam.ca.gc.bdm.application.impl.BDMUtil
          .getAlternateIDByConcernRoleIDType(anyLong,
    ...
    final Entity mailingEntity = personEntity_Applicant.getChildEntities(
      datastore.getEntityType(BDMDatastoreConstants.MAILING_ADDRESS))[0];

    assertEquals(
      personEntity_Applicant
        .getTypedAttribute(BDMDatastoreConstants.LAST_NAME_AT_BIRTH),
      personDtls.personBirthName);
    assertEquals(
      personEntity_Applicant
        .getTypedAttribute(BDMDatastoreConstants.PARENT_LAST_NAME_AT_BIRTH),
      personDtls.motherBirthSurname);
    assertEquals(
      personEntity_Applicant.getTypedAttribute(BDMDatastoreConstants.SIN),
      "123456789");
    assertEquals(personEntity_Applicant.getTypedAttribute(
      BDMDatastoreConstants.MARITAL_STATUS), personDtls.maritalStatusCode);
    assertEquals(
      personEntity_Applicant
        .getTypedAttribute(BDMOASDatastoreConstants.COUNTRY_OF_BIRTH),
      personDtls.countryOfBirth);
    assertEquals(
    ...
    assertEquals(
      ...
      "test string here");
    assertEquals(mailingEntity.getAttribute(BDMDatastoreConstants.SUITE_NUM),
      addressElementApt.elementValue);
    assertEquals(mailingEntity.getAttribute(BDMDatastoreConstants.CITY),
      addressElementCity.elementValue);
    assertEquals(
      mailingEntity.getAttribute(BDMDatastoreConstants.ADDRESS_COUNTRY),
      addressElementCoun.elementValue);
    assertEquals(mailingEntity.getAttribute(BDMDatastoreConstants.PROVINCE),
      addressElementProv.elementValue);
    assertEquals(
      mailingEntity.getAttribute(BDMDatastoreConstants.POSTAL_CODE),
      addressElementPost.elementValue);
    assertEquals(
      mailingEntity.getAttribute(BDMDatastoreConstants.ADDRESS_LINE1),
      addressElementLin1.elementValue);
    assertEquals(
      mailingEntity.getAttribute(BDMDatastoreConstants.ADDRESS_LINE2),
      addressElementLin2.elementValue);
    assertEquals(
      mailingEntity.getAttribute(BDMDatastoreConstants.ONE_LINE_ADDRESS),
      "test string here");

 ...
 * @curam.exclude
 */
public class CustomFunctionValidateIncomeYearBeforeBirthYear
  extends BDMFunctor {

  private static final String kNumberPattern = "^(0|[0-9][0-9]*)$";

  /**
   * A custom function that will validate provided income year cannot be before
   * birth year.
   *
  ...
  public Adaptor getAdaptorValue(final RulesParameters rulesParameters)
    ...
    if (isNumericValueValid(incomeYearStr)
      ...
      && !StringUtil.isNullOrEmpty(birthYearStr)) {
      final int incomeYear = Integer.parseInt(incomeYearStr);
      final int birthYear = Integer.parseInt(birthYearStr);

      if (incomeYear < birthYear) {
        result = false;
      }
    }

...
public class BDMOASEvidenceMappingUtil {
  ...
  private void createConsentDeclarationEvidence(final long concernRoleID,
    ...
    if (declarationEntities != null && declarationEntities.length > 0) {
      ...
      boolean createEvidenceInd = false;
      ...
      if (!StringUtil.isNullOrEmpty(didApplicantSigned)
        || !StringUtil.isNullOrEmpty(didAplicntSignedWithMark)) {
        createEvidenceInd = true;
        ...
        final String didSpouseOrCLPSignedWithMark =
          declarationEntity.getAttribute(
            BDMOASDatastoreConstants.kDidSpouseOrCLPSignedWithMark);

        if (!StringUtil.isNullOrEmpty(didSpouseOrCLPSigned)
          || !StringUtil.isNullOrEmpty(didSpouseOrCLPSignedWithMark)) {
          createEvidenceInd = true;
          ...
          final Date dateSpouseOrCLPSigned =
            StringUtil.isNullOrEmpty(dateSpouseOrCLPSignedStr)
              ? Date.kZeroDate : Date.getDate(dateSpouseOrCLPSignedStr);
      ...
      if (!StringUtil.isNullOrEmpty(didTPSignedAsWitness)) {
        ...
        if (IEGYESNO.YES.equals(didTPSignedAsWitness)) {
      ...
      if (!StringUtil.isNullOrEmpty(didTPSignedBehalfOfAplcnt)) {
        ...
        if (IEGYESNO.YES.equals(didTPSignedBehalfOfAplcnt)) {
          consentAndDeclarationVO.setIsWitnessInd(true);
      ...
      if (!StringUtil.isNullOrEmpty(didTPSignedBehalfOfSpouse)) {
        ...
        if (IEGYESNO.YES.equals(didTPSignedBehalfOfSpouse)) {
          consentAndDeclarationVO.setIsWitnessInd(true);
      ...
      if (createEvidenceInd) {
        ...
        BDMEvidenceUtil.createACOrICDynamicEvidence(concernRoleID,
          evidenceData, CASEEVIDENCE.BDM_CONSENT_DECLARATION,
          EVIDENCECHANGEREASON.INITIAL, caseID);
      }
    }
  }

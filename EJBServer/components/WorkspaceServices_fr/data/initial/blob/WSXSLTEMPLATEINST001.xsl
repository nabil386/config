...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
  ...
  <xsl:template match="DATA">
    ...
    <fo:root>
      ...
      <fo:page-sequence master-reference="default-page" initial-page-number="1" format="1">
        ...
        <fo:static-content flow-name="xsl-region-after" font-family="WT Sans" font-size="8pt">
          <fo:block space-before="5mm" text-align="center">
            Page <fo:page-number/> de <fo:page-number-citation ref-id="terminator"/>
          </fo:block>
        </fo:static-content>
        ...
        <fo:flow flow-name="xsl-region-body">
          <fo:block>
            <fo:block font-family="WT Sans" space-before.optimum="1pt" space-after.optimum="2pt">
              <!-- Start content -->
        ...
        <xsl:when test="(position() mod 2) = 1">
          <fo:table-row>
            ...
            <fo:table-cell padding="2pt" border="none" background-color="#ecedee" display-align="after">
              <fo:block font-family="WT Sans" text-align="right">
  ...
  <xsl:template match="entity">
    <xsl:choose>
      ...
      <xsl:otherwise>        
        <fo:block text-align="left" font-family="WT Sans" font-size="16pt" font-weight="bold" background-color="#dfe8f2" color="#3276ad">
          <fo:leader leader-pattern="space"/>
          <fo:leader leader-pattern="space"/>
    ...
    <xsl:choose>
            ...
            <fo:table-cell number-columns-spanned="{$numCols}" padding="2pt" border="none">
              <xsl:choose>
                <xsl:when test="$level = 1">
                  <fo:block font-family="WT Sans" font-weight="bold" font-size="14pt" color="#3276ad">
                    <xsl:apply-templates select="@description"/>
                  </fo:block>
                </xsl:when>
                <xsl:otherwise>
                  <fo:block font-family="WT Sans" font-weight="bold" font-size="12pt" color="#3276ad">
                    <xsl:apply-templates select="@description"/>
                  </fo:block>
                </xsl:otherwise>
              </xsl:choose>            
      ...
      <xsl:otherwise>        
        ...
        <xsl:if test="count(entities/entity[@hidden = 'false']) &gt; 0">
          ...
          <fo:block font-family="WT Sans" text-align="right" font-size="14pt" font-weight="bold" color="#3276ad">
            <xsl:apply-templates select="@description"/>
          </fo:block>        
        </xsl:if>        
  ...
  <xsl:template name="findRefEntity">
    ...
    <xsl:for-each select="/DOCUMENT/DATA//entity[@name=$refEntityName and attributes/attribute/@name=$refAttributeName and attributes/attribute/@value=$refAttributeValue]">
      ...
      <fo:block font-family="WT Sans" font-size="7pt" font-style="italic">
        <fo:leader leader-pattern="space"/>
        <fo:table table-layout="fixed" width="100%">
          <!-- apply indents if needed -->
          ...
          <fo:table-header>
            <fo:table-row>
              ...
              <fo:table-cell number-columns-spanned="{$numCols}" background-color="#ecedee" padding="2pt" border="none">
                <fo:block font-family="WT Sans" font-weight="bold" font-size="10pt" color="#3276ad">
                  <xsl:value-of select="@description"/>
                </fo:block>
              </fo:table-cell>
            </fo:table-row>

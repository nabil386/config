 ...
 * @curam.exclude
 */
public class CustomFunctionValidateAddress extends BDMFunctor {
   ...
   */
  public CustomFunctionValidateAddress() {
  ...
  private Adaptor validateAddress(final RulesParameters rulesParameters)
    ...
    final Entity[] mailingAddressArray = personEntity
      .getChildEntities(datastore.getEntityType("MailingAddress"));
    ...
    if (country != null && !country.equals("")
      ...
      if (!isWSValidationEnabled()) {
        mailingAddress.setAttribute("validateStatus", "1");
        return AdaptorFactory.getBooleanAdaptor(VALID_ADDRESS);

      }

      // is the address manual, or is this the 2nd attempt to validate
      // BEGIN TASK- 28743 Validate Manual Address
      /*
       * if (isAddressManual(id) || isAddressEmpty(city, province, postalCode))
       * {
       * mailingAddress.setAttribute("validateStatus", "1");
       * return AdaptorFactory.getBooleanAdaptor(VALID_ADDRESS);
       * } else {
      ...
      try {
        ...
        if (streetName != null) {
          ...
          if (returnValue) {
            ...
            mailingAddress.setAttribute("validateStatus", "2");
          }
        } else { // when PO box address is entered
          mailingAddress.setAttribute("validateStatus", "1");
        }
        ...
        if (BDMRESTAPIERRORMESSAGE.BDM_ERR_WS_ADDRESS_INVALID_EXCEPTION
          ...
          mailingAddress.setAttribute("validateStatus", "3");
        }
        //
        e.printStackTrace();
      }

...
public class CustomFunctionvalidateAlertsForAP extends CustomFunctor {
  ...
  public Adaptor getAdaptorValue(final RulesParameters rulesParameters)
    ...
    if (receiveAlertYesNo != null && receiveAlertYesNo.equals(IEGYESNO.YES)) {
      final java.util.List<java.lang.Long> selectedEmailEntityIDList =
        ieg2Context.getListQuestionSelectedEntityIDs("emailAdrSelected",
          BDMDatastoreConstants.COMMUNICATION_DETAILS);

      final java.util.List<java.lang.Long> selectedPhoneEntityIDList =
        ieg2Context.getListQuestionSelectedEntityIDs("phoneNumberSelected",
          BDMDatastoreConstants.COMMUNICATION_DETAILS);

      if (!selectedEmailEntityIDList.isEmpty()
        || !selectedPhoneEntityIDList.isEmpty()) {
...
/*
 * }
 * if (checkCommunicationPref(BDMDatastoreConstants.ALT_PREF_EMAIL,
 * ieg2Context)
 * || checkCommunicationPref(BDMDatastoreConstants.PHONE_ALT_PREF,
 * ieg2Context)) {
 * return AdaptorFactory.getBooleanAdaptor(true);
 * }
 * return AdaptorFactory.getBooleanAdaptor(false);

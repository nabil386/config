      ...
      try {
        ...
        if (!nfIndicator.isNotFound()) {
          ...
          if (bdmPmt.reconcilStatusCode
            ...
            bdmPaymentInstrumentKey.pmtInstrumentID = bdmPmt.pmtInstrumentID;
            ...
            if (prnStageDtls.paymentInstrumentType == BDMConstants.PAYMENT_TYPE_CHEQUE) {
              bdmPmtInstrDtls.spsDeliveryMethodType = METHODOFDELIVERY.CHEQUE;
            } else {
              bdmPmtInstrDtls.spsDeliveryMethodType = METHODOFDELIVERY.EFT;
            }
            ...
            dataDtls = pmtReturnData.read(dataKey);
            dataDtls.processingDateTime = DateTime.getCurrentDateTime();
            dataDtls.processingStatus = BATCHPROCESSRESULTSTATUS.PROCESSED;
            pmtReturnData.modify(dataKey, dataDtls);

          } else {
            Trace.kTopLevelLogger.error("Payment with ID - '"
              + bdmPmt.pmtInstrumentID + "' is not in transferred status.");
          }

        } else {
          // If a payment is not found
          Trace.kTopLevelLogger.error("Payment with ID - '"
            + instrDtls.pmtInstrumentID + "' is not found in the database.");
        }

...
public class MaintainCaseDeductions implements
   ...
   * @throws InformationalException
   */
  private void recordVTWDeductionByPDC(final CaseIDKey caseIDKey,
    ...
    if (deductionName != null
      ...
      if (!caseDeductionItemDtls.startDate.after(Date.getCurrentDate())
        && (vtwDeductionDtls.endDate.isZero()
          || !vtwDeductionDtls.endDate.before(Date.getCurrentDate()))) {
        this.activateDeductionItem(caseDeductionItemKey);
      }
  ...
  public BDMCaseDeductionItemKey createThirdPartyCaseDeduction(
    ...
    if (endDate != null && !endDate.isZero()) {
      if (concernRoleDtls.endDate.before(endDate)) {
  ...
  public void createTaxDeductions(final CaseIDKey key)
    ...
    boolean deductionCreated = false;
    DeductionName deductionName = null;
    final CreateCaseDeductionDetails createCaseDeductionDetails =
      new CreateCaseDeductionDetails();
    final BDMCaseDeductionItemDA bdmCaseDeductionItemDA =
    ...
    try {
      boolean validDeductionExists = true;
      ...
      try {
        ...
        validDeductionExists = false;
      }

      // If deduction type is configured for product and no existing case
      // deduction item exists for the given case,
      // create new Case Deduction Item for this case.
      if (!validDeductionExists) {
        getDeductionNameByCaseKey.caseID = key.caseID;
        getDeductionNameByCaseKey.deductionType =
          BDMDEDUCTIONTYPE.BASIC_FEDERAL_TAX;
        deductionName =
          bdmDeductionObj.readDeductionNameByCase(getDeductionNameByCaseKey);
        ...
        createCaseDeductionDetails.rate = 0.01;
        createThirdPartyCaseDeduction(createCaseDeductionDetails,
          new BDMExternalLiabilityKey());
      }
      deductionCreated = true;
    } catch (final RecordNotFoundException rnfex) {
      // Product is not configured to create auto case deduction item for Basic
      // Federal Tax.
    }
   ...
   * @throws InformationalException
   */
  private void activateDeductionItem(final CaseDeductionItemKey key)
    throws AppException, InformationalException {

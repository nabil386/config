...
public class BDMPDCPerson
  ...
  public void changeIdentificationInfoDetails(
    ...
    if (!details.dtls.sinNumber.isEmpty()
      ...
      if (details.dtls.sinIdentificationEndDate.isZero()
        && !details.dtls.sinIdentificationStartDate.isZero()
        && !details.dtls.sinIdentificationExistingStartDate.isZero()
        && (!details.dtls.sinIdentificationStartDate
          .equals(details.dtls.sinIdentificationExistingStartDate)
          || details.dtls.sinIdentificationStartDate
            .before(details.dtls.sinIdentificationExistingStartDate))) {

        final LocalisableString localisableString = new LocalisableString(
          BDMIDENTIFICATIONINFO.START_DATE_OF_THE_NEW_SIN_MUST_BE_AFTER_END_DATE_OF_THE_PRIOR_SIN);
  ...
  public BDMPDCEvidenceDetailsList
    ...
    final CuramInd originalConcernhadDupInd =
      clientMergeObj.isConcernRoleOriginalClient(concernRoleIDStatusCodeKey);
    ...
    for (int i = 0; i < pdcEvidenceList.list.size(); i++) {
      ...
      if ((pdcEvidenceList.list.get(i).evidenceType
        .equals(CASEEVIDENCE.BDMADDRESS)
        || pdcEvidenceList.list.get(i).evidenceType.equals(CASEEVIDENCE.NAMES)
        || pdcEvidenceList.list.get(i).evidenceType
          .equals(CASEEVIDENCE.IDENTIFICATIONS)
        || pdcEvidenceList.list.get(i).evidenceType
          .equals(CASEEVIDENCE.BDM_MARITAL_STATUS))
        && originalConcernhadDupInd.statusInd) {

        final String latestactivity =
          pdcEvidenceList.list.get(i).latestActivity;

        final String latestactivity1 =
          latestactivity.substring(0, latestactivity.indexOf(":("));
        final String latestactivity2 = latestactivity1.concat(":(Merge");
        final String latestactivity3 = latestactivity2
          .concat(latestactivity.substring(latestactivity.indexOf("|")));
        pdcEvidenceList.list.get(i).latestActivity = latestactivity3;
      }
    }

...
public class BDMFIFInboundBatch
  ...
  public void dbBatchReport() throws AppException, InformationalException {
    ...
    final BDMFIFReport bdmfifReport4 = new BDMFIFReport();
    DynamicDataAccess.executeNs(BDMFIFReport.class, null, false,
      bankCountSQL);
    Trace.kTopLevelLogger
      .info("Number of Bank count: " + bdmfifReport4.countResult);

    final BDMFIFReport bdmfifReport = new BDMFIFReport();
    DynamicDataAccess.executeNs(BDMFIFReport.class, null, false,
    ...
    final BDMFIFReport bdmfifReport3 = new BDMFIFReport();
    DynamicDataAccess.executeNs(BDMFIFReport.class, bdmfifReport3, false,
      uniqueBankNamesSQL);
    Trace.kTopLevelLogger
      .info("Number of distinct Bank names: " + bdmfifReport3.countResult);

    final BDMFIFReport bdmfifReport1 = new BDMFIFReport();
    DynamicDataAccess.executeNs(BDMFIFReport.class, bdmfifReport, false,
      blankAdressSQL);
    Trace.kTopLevelLogger.info(
      "Number of records with blank address: " + bdmfifReport1.countResult);

    final BDMFIFReport bdmfifReport2 = new BDMFIFReport();
    DynamicDataAccess.executeNs(BDMFIFReport.class, bdmfifReport, false,
      blankPOCodeSQL);
    Trace.kTopLevelLogger.info(
      "Number of records with blank po code: " + bdmfifReport2.countResult);

    final BDMFIFReport bdmfifReport5 = new BDMFIFReport();
    DynamicDataAccess.executeNs(BDMFIFReport.class, bdmfifReport5, false,
      skippedRecordSQL);
  ...
  public void getSQL() {
    ...
    blankAdressSQL =
      "SELECT 'Number of records with address is blank',count(*) FROM BDMFIFSTAGEINBOUND INTO :recordDetails, :countResult WHERE branchstatus='V' and BRANCHCIVICADDRESS is NULL AND BRANCHPOADDRESS is NULL";

    blankPOCodeSQL =
      "SELECT 'Number of records with blank PO Code',count(*) FROM BDMFIFSTAGEINBOUND INTO :recordDetails, :countResult WHERE branchstatus='V' and branchpocode is NULL";
    ...
    bankCountSQL =
      "SELECT 'Number of bank records - matches with FIF Stage',count(*) FROM BDMFIFSTAGEINBOUND INTO :recordDetails, :countResult WHERE  banksortcode in (SELECT banksortcode FROM BDMFIFSTAGEINBOUND)";

    skippedRecordSQL =
      "SELECT 'Number of records skipped',count(*) FROM BDMFIFSTAGEINBOUND INTO :recordDetails, :countResult WHERE banksortcode NOT IN (SELECT banksortcode FROM BDMFIFSTAGEINBOUND)";

    skippedRecordByStatusSQL =
      "SELECT 'Number of records skipped - by status',branchstatus,count(*) FROM BDMFIFSTAGEINBOUND  WHERE banksortcode NOT IN (SELECT banksortcode FROM BDMFIFSTAGEINBOUND) GROUP BY branchstatus";

...
public class BDMAddressEvidenceWizard
  ...
  private void nextPageOperation(
    ...
    if (wizardDtls.currentStepNum == 1) {
      ...
      wizardDtls.step3DataDtls.clientSelectedCity =
        trimAddressField(getCityNameForAllAddress(
          addressEvidenceWizardDetailsResult.step2DataDtls,
  ...
  private void processAddressEvdAndValidateAddress(
    ...
    if (!addressEvidenceWizardDetailsResult.step2DataDtls.country
      ...
      otherAddressData.addressData =
        bdmUtil.setBDMUnparsedIndForINTLAddress(otherAddressData.addressData);
    }
    // END TASK-118752
    ...
    if (actionID.actionIDProperty.equals("SEARCH")) {
      ...
      if (!wizardDtls.step2DataDtls.country.equals("")) {
        ...
        if (wizardDtls.currentStepNum == 1) {
          ...
          final OtherAddressData otherAddressData =
            addressDataObj.parseFieldsToData(addressFieldDetails);
          // START : TASK-118752 : NEW Common Code Set the BDMUNPARSE INDICATOR
          // for intl address
          otherAddressData.addressData = bdmUtil
            .setBDMUnparsedIndForINTLAddress(otherAddressData.addressData);
          final Address addressObj = AddressFactory.newInstance();
          final OtherAddressData formattedAddressData =
            addressObj.getShortFormat(otherAddressData);
          addressEvidenceWizardDetailsResult.step4DataDtls.formattedAddressData =
  ...
  private void validateAddressFormatLength(
    ...
    if ((isCAAddress || isUSAddress
      ...
      // TASK 118752 : Validation for International Address
      boolean validationExists = false;
      final String intlAddressLine1 =
        addressEvidenceWizardStp3DataDetails.clientSelectedUnitNumber.trim();
      final String intlAddressLine2 =
        addressEvidenceWizardStp3DataDetails.clientSelectedAddressLine1
          .trim();
      final String intlAddressLine3 =
        addressEvidenceWizardStp3DataDetails.clientSelectedAddressLine2
          .trim();
      // Address Line 1 must be between 1 to 40 characters.
      if (intlAddressLine1.isEmpty()
        || intlAddressLine1.length() > addresLineLimit) {
        ValidationManagerFactory.getManager().addInfoMgrExceptionWithLookup(
          new AppException(
            BDMBPOADDRESS.ERR_INTL_ADDRESS_LINE_ONE_LENGTH_VALIDATION),
          null, InformationalType.kError,
          curam.core.sl.infrastructure.impl.ValidationManagerConst.kSetNine,
          0);
        validationExists = true;
      }
      // Address Line 2 has a maximum of 40 characters.
      if (intlAddressLine2.length() > addresLineLimit) {
        ValidationManagerFactory.getManager().addInfoMgrExceptionWithLookup(
          new AppException(
            BDMBPOADDRESS.ERR_INTL_ADDRESS_LINE_TWO_LENGTH_LIMIT),
          null, InformationalType.kError,
          curam.core.sl.infrastructure.impl.ValidationManagerConst.kSetNine,
          0);
        validationExists = true;
      }
      // Address Line 3 has a maximum of 40 characters.
      if (intlAddressLine3.length() > addresLineLimit) {
        ValidationManagerFactory.getManager().addInfoMgrExceptionWithLookup(
          new AppException(
            BDMBPOADDRESS.ERR_INTL_ADDRESS_LINE_THREE_LENGTH_LIMIT),
          null, InformationalType.kError,
          curam.core.sl.infrastructure.impl.ValidationManagerConst.kSetNine,
          0);
        validationExists = true;
      }
      // Fail the operation is there is one or more validation.
      if (validationExists) {
        TransactionInfo.getInformationalManager().failOperation();
      }
    }
  ...
  /**
   * TASK 118752 : Set the country description into the field city for
   * international address
   *
   * @param addressEvidenceWizardStp2DataDetailsResult
   * @param addressEvidenceWizardStp3DataDetails
   * @return
   * @throws AppException
   * @throws InformationalException
   */
  private String getCityNameForAllAddress(
    final AddressEvidenceWizardStp2DataDetailsResult addressEvidenceWizardStp2DataDetailsResult,
    final AddressEvidenceWizardStp3DataDetails addressEvidenceWizardStp3DataDetails)
    throws AppException, InformationalException {
    ...
    if (isCAAddress || isUSAddress
      || addressEvidenceWizardStp2DataDetailsResult.isUSAddress
      || addressEvidenceWizardStp2DataDetailsResult.isCAAddress) {
      return addressEvidenceWizardStp3DataDetails.clientSelectedCity;
    } else { // international address
      // return the country description
      return CodeTable.getOneItem(COUNTRY.TABLENAME,
        addressEvidenceWizardStp3DataDetails.clientSelectedCity,
        LOCALE.ENGLISH);
    }
  }
}

...
/**
 * This class is the chunker of for the BDM Citizen Workspace Purge In Progress
 * Data Process Batch. This chunker gets all of the in-progress draft
 * applications which has a last updated date before 7 days than the current
 * date.
 *
 * @author alim.maredia
 *
 */
public class BDMCitizenWorkspacePurgeInProgressDataProcess extends
  curam.ca.gc.bdm.batch.bdmcitizenworkspacepurgeinprogressdataprocess.base.BDMCitizenWorkspacePurgeInProgressDataProcess {
  ...
  public void process(
    ...
    final BDMCitizenWorkspacePurgeInProgressDataProcessWrapper chunkWrapper =
      new BDMCitizenWorkspacePurgeInProgressDataProcessWrapper(this);
    // Instantiate the streamWrapper
    final BDMCitizenWorkspacePurgeInProgressDataProcessStreamWrapper streamWrapper =
      new BDMCitizenWorkspacePurgeInProgressDataProcessStreamWrapper(
        new BDMCitizenWorkspacePurgeInProgressDataProcessStream());
  ...
  public BatchProcessingIDList getBatchProcessingIDs()
    ...
    final BDMCitizenScriptInfoReadMultiOperation submittedIntakeReadMultiOp =
      new BDMCitizenScriptInfoReadMultiOperation(idList);
    // Specify that the id's should only be within 31 days in the past from the
    ...
    final int numberOfDays = Configuration.getIntProperty(
      EnvVars.BDM_CITIZEN_WORKSPACE_PURGE_IN_PROGRESS_DATA_PROCESS_NO_OF_DAYS)
      - 1;
    statusKey.status = CitizenScriptStatusEntry.IN_PROGRESS.getCode();
    statusKey.date = Date.getCurrentDate().addDays(-numberOfDays);
    statusKey.scriptType = OnlineScriptTypeEntry.INTAKE.getCode();
  ...
  private ChunkMainParameters getChunkMainParameters() {
    ...
    chunkMainParameters.chunkSize = Configuration.getIntProperty(
      EnvVars.BDM_CITIZEN_WORKSPACE_PURGE_IN_PROGRESS_DATA_PROCESS_CHUNK_SIZE);
    ...
    chunkMainParameters.dontRunStream = Configuration.getBooleanProperty(
      EnvVars.BDM_CITIZEN_WORKSPACE_PURGE_IN_PROGRESS_DATA_PROCESS_DONT_RUN_STREAM);
    ...
    chunkMainParameters.startChunkKey =
      Long.parseLong(Configuration.getProperty(
        EnvVars.BDM_CITIZEN_WORKSPACE_PURGE_IN_PROGRESS_DATA_PROCESS_START_CHUNK_KEY));
    // Get the wait time for unprocessed chunks
    chunkMainParameters.unProcessedChunkReadWait =
      Configuration.getIntProperty(
        EnvVars.BDM_CITIZEN_WORKSPACE_PURGE_IN_PROGRESS_DATA_PROCESS_UNPROCESSED_CHUNK_READ_WAIT);
    ...
    chunkMainParameters.processUnProcessedChunks =
      Configuration.getBooleanProperty(
        EnvVars.BDM_CITIZEN_WORKSPACE_PURGE_IN_PROGRESS_DATA_PROCESS_UNPROCESSED_CHUNKS);

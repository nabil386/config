...
import curam.ca.gc.bdm.facade.struct.BDMTaxSlipUpdateDetails;
import curam.ca.gc.bdm.facade.struct.BDMTaxSlipUpdateDetailsList;
 ...
 *
 */
public class BDMTaxSlipUpdateAPI
  extends curam.ca.gc.bdm.facade.base.BDMTaxSlipUpdateAPI {

  public BDMTaxSlipUpdateAPI() {

    GuiceWrapper.getInjector().injectMembers(this);
  }

  @Override
  public void updateTaxSlipProcStatus(
    final BDMTaxSlipUpdateDetailsList taxSlipUpdateList)
    ...
    for (final BDMTaxSlipUpdateDetails taxSlipDetails : taxSlipUpdateList.taxSlipUpdateDetails) {
      ...
      if (taxSlipDetails.taxSlipFormType != null
        ...
        // If the tax slip status is 'Issued', throw error
        if (taxSlipDataT4ADtls.taxSlipStatusCode
          .equals(BDMTAXSLIPSTATUS.ISSUED)) {
          final AppException exceptionErr = new AppException(
            BDMRESTAPIERRORMESSAGE.BDM_ERR_TAX_SLIP_INVALID_STATUS);
          Trace.kTopLevelLogger.debug(
            "TaxSlipUpdateAPI invalid status: " + exceptionErr.getMessage());
          throw exceptionErr;
        }
        ...
        // If the tax slip status is 'Issued', throw error
        if (taxSlipDataRL1DtlsObj.taxSlipStatusCode
          .equals(BDMTAXSLIPSTATUS.ISSUED)) {
          final AppException exceptionErr = new AppException(
            BDMRESTAPIERRORMESSAGE.BDM_ERR_TAX_SLIP_INVALID_STATUS);
          Trace.kTopLevelLogger.debug(

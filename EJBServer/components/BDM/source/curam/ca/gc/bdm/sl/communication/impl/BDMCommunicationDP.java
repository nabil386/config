...
import curam.ca.gc.bdm.entity.communication.struct.BDMCorrespondenceStatusJobID;
...
public class BDMCommunicationDP
  ...
  @Override
  /**
   * This method updates the status of all communicationRecords associated
   * with the jobID
   *
   * @param long ticketID
   * @param long instanceDataID
   * @param boolean flag
   */
  public void updateJobStatusDP(final long ticketID, final long inst_data_id,
    ...
    final WMInstanceDataDtls wmInstanceDataDtls =
      wmInstanceDataObj.read(nfIndicator, wmInstanceDataKey);
    if (!nfIndicator.isNotFound()) {
      // the jobID would be comments
      // the jobStatus would be the caseStatus
      final long jobID = Long.parseLong(wmInstanceDataDtls.comments);

      final BDMCorrespondenceStagingStatus bdmCorrespondenceStagingStatus =
      ...
      final BDMCorrespondenceStagingKeyList keyList =
        BDMCorrespondenceStagingFactory.newInstance().searchByJobID(key);
      if (!keyList.dtls.isEmpty()) {
        bdmCorrespondenceStagingStatus.status =
          CORRESPONDENCEJOBSTATUS.PRINTED;
        BDMCorrespondenceStagingFactory.newInstance().modifyStatusByJobID(key,
          bdmCorrespondenceStagingStatus);

        final BDMCorrespondenceStatusJobID corrStatusJobID =
          new BDMCorrespondenceStatusJobID();
        corrStatusJobID.jobID = key.jobID;
        corrStatusJobID.status = COMMUNICATIONSTATUS.SENT;
        BDMConcernRoleCommunicationSubFactory.newInstance()
          .modifyCommunicationStatusByJobID(corrStatusJobID);

        corrStatusJobID.status = BDMCCTCOMMUNICATIONSTATUS.FINISHED;
        BDMConcernRoleCommunicationFactory.newInstance()
          .modifyCCTStatusByJobID(corrStatusJobID);
      }
    }

...
import curam.ca.gc.bdm.entity.struct.BDMBankBranchReadByBICLocaleKey;
...
public class Statics {
  ...
  public static String getBankNameUsingInstitutionNumberAndTransitNumber(
    ...
    String bankName = "";
    final BDMBankBranchReadByBICLocaleKey keyStruct1 =
      new BDMBankBranchReadByBICLocaleKey();
  ...
  public static boolean isPercentageOrAmountIsChanged(final Session session,
    ...
    final Number amountValue = (Number) vtwEvd
      .getAttributeValue("voluntaryTaxWithholdAmount").getValue();

    final boolean isDollarAmountExists = amountValue.doubleValue() > 0.0;

    final Number percentageValue = (Number) vtwEvd
      .getAttributeValue("voluntaryTaxWithholdPercentage").getValue();
    ...
    for (final ECActiveEvidenceDtls activeEvidenceDtls : bankAccountEvdList.dtls) {
      ...
      if (activeEvidenceDtls.successionID == successionID
        ...
        final String earlierPercentageValue =
          BDMEvidenceUtil.getDynEvdAttrValue(activeEvidenceDtls.evidenceID,
            evidenceKey.evidenceType, "voluntaryTaxWithholdPercentage");
        final String earlierDollarAmounteValue =
          BDMEvidenceUtil.getDynEvdAttrValue(activeEvidenceDtls.evidenceID,
            evidenceKey.evidenceType, "voluntaryTaxWithholdAmount");
  ...
  /**
   * Method checks value for only the alpha numeric characters
   *
   * @param session
   * Rules Session
   *
   * @param value
   * Contains the String to be validated for the only
   * alpha numeric characters.
   * @return
   * <code>true</code> if value contains only the alpha numeric characters
   * otherwise <code>false</code>
   */
  public static boolean isAlphaNumericOnly(final Session session,
    final String value) {

    return value.matches(BDMConstants.ALPHA_NUMERIC_PATTERN);

  }

  /**
   * Shifts a numeric timeline to the start of the subsequent month. The final
   * value within the current month is used. Any earlier values are ignored.
   *
   * @param session
   * @param timeline
   * @return
   */
  public static Timeline<Number> getSubsequentMonthNumberTimeline(
    final Session session, final Timeline<Number> timeline) {

    final List<Interval<Number>> intervals = new LinkedList<>();
    final Iterator<? extends BoundedInterval<Number>> iterator =
      timeline.intervals().iterator();

    intervals.add(iterator.next());

    final Map<Date, Number> dateValueMap = new HashMap<>();

    while (iterator.hasNext()) {

      final Interval<Number> interval = iterator.next();
      final Date startDate =
        BDMDateUtil.getStartOfNextMonth(interval.startDate());

      dateValueMap.put(startDate, interval.value());
  ...
  /**
   * This method validates if the year is valid income year.
   *
   * @param session
   * @param year
   * @return
  ...
  public static boolean isValidYear(final Session session,
    ...
    if (!StringUtil.isNullOrEmpty(year)
      ...
      if (yearValue >= 1967 && yearValue <= previousYear) {
  ...
  /**
   * This method will return the previous calendar year.
   *
   * @param session
   * Creole Rules session.
   * @return
   * returns the previous calendar year.
   */
  public static String getPreviousYear(final Session session) {

    return Integer
      .toString(Date.getCurrentDate().getCalendar().get(Calendar.YEAR)
        - CuramConst.gkOne);
  }

  /**
   * Returns Year String value "YYYY" from the input date.
   *
   * @param session
   * @param date
   * @return
   */
  public static String getYearFromDate(final Session session,
    final Date date) {

    return String.valueOf(date.getCalendar().get(Calendar.YEAR));
  }
  ...
  // END 73158 DEV: Contribution details Evidence in Foreign Engagement Case

  // BEGIN 76342: Application Life Cycle Evidence
  /**
   * Determines whether the given date falls on the first of the month in
   * question.
   *
   * @param session
   * @param date
   * @return
   */
  public static Boolean isFirstOfMonth(final Session session,
    final Date date) {

    return BDMDateUtil.isFirstOfMonth(date);

  }
  // END 76342: Application Life Cycle Evidence

  // BEGIN Feature 99661 : Implement Manage VTW Evidence
   ...
   * @param numMonths months to add to date
   * @return New date numMonths after specified date
   */
  public static Date addMonthsToDate(final Session session, final Date date,
    final Number numMonths) {

    Date shiftedDate = null;
    if (date != null) {

      final Calendar shiftedDateCalendar = date.getCalendar();
      shiftedDateCalendar.add(Calendar.MONTH, numMonths.intValue());
  ...
  /**
   * This method will check if the Provincial VTW evidence
   * is valid based on the residential address
   *
   * @param session
   * CREOLE session
   * @param participantRoleID
   * Participant Role ID value
   *
   * @return
   * <code>true</code> if the valid PVTW evidence, <code>false</code>
   * otherwise.
   *
   * @throws AppException
   ...
   */
  public static Boolean hasValidAddressForPVTW(final Session session,
    final Number participantRoleID, final String provinceType,
    final Date vtwStartDate, final Date vtwEndDate)
    throws AppException, InformationalException {
    ...
    for (final DynamicEvidenceDataDetails addressDynamicEvidenceDataDetails : addressEvdDtls) {
      ...
      if (addressDynamicEvidenceDataDetails.getAttribute("addressType")
        ...
        if (country.equals(COUNTRYCODE.CACODE)
          ...
          // Check the VTW evidence start and end date with Address From and To
          // Dates.
          if (Date
            ...
            || Date.getDate(addressDynamicEvidenceDataDetails
              .getAttribute("fromDate").getValue()).equals(vtwStartDate)) {

            // If the start Date is valid Check for the end date

            if (StringUtil.isNullOrEmpty(addressDynamicEvidenceDataDetails
              .getAttribute("toDate").getValue())) {

              isValidProvinceTypeResident = true;
              return isValidProvinceTypeResident;

            } else if (vtwEndDate != null && (vtwEndDate
              .before(Date.getDate(addressDynamicEvidenceDataDetails
                .getAttribute("toDate").getValue()))
              || vtwEndDate
                .equals(Date.getDate(addressDynamicEvidenceDataDetails
                  .getAttribute("toDate").getValue())))) {
  ...
  /**
   * This method will read the maximum amount configured for the
   * deduction.
   *
   * @param session
   * CREOLE Session
   * @param deductionName
   * Deduction Name code value.
   * @return
   * Maximum Amount configured for the deduction.
   */
  public static Double readMaxAmountForDeduction(final Session session,
    final CodeTableItem deductionName)
    throws AppException, InformationalException {

    DeductionDtls deductionDtls = new DeductionDtls();
    ...
    deductionNameStatus.deductionName = deductionName.code();
    deductionNameStatus.recordStatus = RECORDSTATUS.NORMAL;

    deductionDtls = DeductionFactory.newInstance()
      .readActiveDeductionByName(notFoundIndicator, deductionNameStatus);

    return deductionDtls.maxAmount.getValue();

  }

  /**
   * This method will read the minimum amount configured for the
   * deduction.
   *
   * @param session
   * CREOLE Session
   ...
   * Deduction Name code value.
   * @return
   * Minimum Amount configured for the deduction.
   */
  public static Double readMinimumAmountForDeduction(final Session session,
    final CodeTableItem deductionName)
    throws AppException, InformationalException {
    ...
    return deductionDtls.minAmount.getValue();

  }

  /**
   * This method will read the maximum percentage configured for the
   * deduction.
   *
   * @param session
   * CREOLE Session
   ...
   * Maximum Amount configured for the deduction.
   */
  public static Double readMaxPercentageForDeduction(final Session session,
    final CodeTableItem deductionName)
    throws AppException, InformationalException {
    ...
    return deductionDtls.maxPercentage;

  }

  // END Feature 99661 : Implement Manage VTW Evidence

  public static Date getEarliestTrueDate(final Session session,
    final Timeline<Boolean> timeline) {

    Date earliestTrueDate = Date.kZeroDate;

    for (final Interval<Boolean> interval : timeline.intervals()) {
      if (interval.value()) {
        earliestTrueDate = interval.startDate();
        break;
      }
    }

    return earliestTrueDate;

  }

  public static Date getLatestTrueDateOnBeforeDate(final Session session,
    final Timeline<Boolean> timeline, final Date date) {

    Date latestTrueDate = Date.kZeroDate;

    for (final BoundedInterval<Boolean> interval : Lists
      .reverse(timeline.intervals())) {
      if (interval.value()) {
        latestTrueDate = interval.endDate();
        break;
      }
    }

    return latestTrueDate;

  }

  public static Date getSubsequentMonthStart(final Session session,
    final Date date) {
  ...
  public static Date addYearsToDate(final Session session, final Date date,
    final Number numYears) {

    final Calendar calendar = date.getCalendar();
    calendar.add(Calendar.YEAR, numYears.intValue());

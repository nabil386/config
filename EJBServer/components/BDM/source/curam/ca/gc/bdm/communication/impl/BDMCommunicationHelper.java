...
public class BDMCommunicationHelper {
  ...
  public long getAddressIDforConcern(final long concernRoleID,
    ...
    final BDMParticipant participant = BDMParticipantFactory.newInstance();
    final ReadParticipantAddressListKey addressListKey =
      new ReadParticipantAddressListKey();
    addressListKey.maintainAddressKey.concernRoleID = concernRoleID;
    final BDMReadParticipantAddressDetails addressDetails =
      participant.readMailingAddress(addressListKey);

    if (0 == addressDetails.addressID) {
      final ConcernRoleKey concernRoleKey = new ConcernRoleKey();
      concernRoleKey.concernRoleID = concernRoleID;
      final ConcernRoleDtls concernRoleDtls =
        ConcernRoleFactory.newInstance().read(concernRoleKey);
      if (isThirdPartyContactInd) {
        addressDetails.addressID = concernRoleDtls.primaryAddressID;
      }
      if (0 == addressDetails.addressID) {

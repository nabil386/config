...
public class BDMCommunication
  ...
  public ConcernRoleCommKeyOut createRecordedCommWithReturningID(
    ...
    // START: Bug 121716: Concernrole ID not being populated and hence we are
    // not able
    // to search for the correspondence that created under Application Case via
    // Correspondence Tracking Number.
    if (recordedCommDetails.caseID != CuramConst.kLongZero
      && concernRoleCommKeyOut.communicationID != CuramConst.kLongZero) {
      final ConcernRoleCommunicationKey concernroleCommunicationKey =
        new ConcernRoleCommunicationKey();
      concernroleCommunicationKey.communicationID =
        concernRoleCommKeyOut.communicationID;

      ConcernRoleCommunicationDtls concernRoleCommunicationDtls2 =
        new ConcernRoleCommunicationDtls();
      final ConcernRoleCommunication concernRoleCommunication =
        ConcernRoleCommunicationFactory.newInstance();

      concernRoleCommunicationDtls2 =
        concernRoleCommunication.read(concernroleCommunicationKey);

      if (concernRoleCommunicationDtls2.concernRoleID == CuramConst.kLongZero) {
        concernRoleCommunicationDtls2.concernRoleID =
          recordedCommDetails.correspondentParticipantRoleID;
        concernRoleCommunication.modify(concernroleCommunicationKey,
          concernRoleCommunicationDtls2);
      }
  ...
  public BDMTemplateDetails
    ...
    if (wizardKey.submitOpt.isEmpty()) {
      ...
      // If template is present in the appresource config then call the
      // correspondence framework code
      if (BDMCorrespondenceTriggerUtil
        .getTemplateConfiguration(corresDetails.templateName) != null
        && !BDMCorrespondenceTriggerUtil
          .getTemplateXML(corresDetails.templateName).isEmpty()) {
        // TODO How to set input for all sql that will be used for the manual
        // letters
        // in
        // a generic way?
        ...
        return BDMCorrespondenceTriggerUtil.generateCorrespondenceForRealTime(
   ...
   * @throws InformationalException
   */
  public long insertCorrespondenceDetails(
    final BDMCorrespondenceDetails commDtls, final String communicationStatus)
    throws AppException, InformationalException {
    ...
    concernRoleCommDtls.communicationStatus = communicationStatus;
    ...
    // TASK-52914, Start
    // Capture To and CC recipient contact types
    if (commDtls.toRecipientContactCode
      .equalsIgnoreCase(BDMConstants.kGenerateCorrespondenceXMLPatronato)) {
      bdmConcernRoleCommDtls.toRecipientContactType =
        BDMRECIPIENTCONTACTTYPE.PATRONATO;
    } else if (commDtls.toRecipientContactCode
      .equalsIgnoreCase(BDMConstants.kGenerateCorrespondenceXMLClient)) {
      bdmConcernRoleCommDtls.toRecipientContactType =
        BDMRECIPIENTCONTACTTYPE.CLIENT;
    } else if (commDtls.toRecipientContactCode
      .equalsIgnoreCase(BDMConstants.kGenerateCorrespondenceXMLIndividual)) {
      bdmConcernRoleCommDtls.toRecipientContactType =
        BDMRECIPIENTCONTACTTYPE.INDIVIDUAL;
    } else if (commDtls.toRecipientContactCode.equalsIgnoreCase(
      BDMConstants.kGenerateCorrespondenceXMLOrganization)) {
      bdmConcernRoleCommDtls.toRecipientContactType =
        BDMRECIPIENTCONTACTTYPE.ORGANIZATION;
    }

    if (commDtls.ccRecipientContactCode
      .equalsIgnoreCase(BDMConstants.kGenerateCorrespondenceXMLPatronato)) {
      bdmConcernRoleCommDtls.ccRecipientContactType =
        BDMRECIPIENTCONTACTTYPE.PATRONATO;
    } else if (commDtls.ccRecipientContactCode
      .equalsIgnoreCase(BDMConstants.kGenerateCorrespondenceXMLClient)) {
      bdmConcernRoleCommDtls.ccRecipientContactType =
        BDMRECIPIENTCONTACTTYPE.CLIENT;
    } else if (commDtls.ccRecipientContactCode
      ...
      bdmConcernRoleCommDtls.ccRecipientContactType =
        BDMRECIPIENTCONTACTTYPE.INDIVIDUAL;
    } else if (commDtls.ccRecipientContactCode.equalsIgnoreCase(
      BDMConstants.kGenerateCorrespondenceXMLOrganization)) {
      bdmConcernRoleCommDtls.ccRecipientContactType =
        BDMRECIPIENTCONTACTTYPE.ORGANIZATION;
    }
    ...
    bdmConcernRoleCommDtls.cctStatus =
      BDMCCTCOMMUNICATIONSTATUS.ACTIVE_INCOMPLETE;
  ...
  public BDMViewCorrespondenceDetails
    ...
    if (CuramConst.gkZero != bdmCommDtls.ccAddressID) {
      ...
      for (final String address : addrString) {
  ...
  public void saveCCTCorrespondenceResponce(
    final BDMCCTCreateCorrespondenceResponse response,
    final BDMCorrespondenceDetails corresDetails)
    throws AppException, InformationalException {

...
public class BDMApplicationEventsUtil {
  ...
  // BEGIN: Task 92423 Refactor foundation code to accommodate the appl scripts
  /**
   * Get the entities of a given root entity ID and entity name.
   *
   * @param rootEntityID
   ...
   * the name of the entity as defined in the schema
   * @param rootEntity
   * the rootEntity as defined in the schema
   * @return
   * an array of entities, which may be empty.
   */
  public static Entity[] getEntities(final long rootEntityID,
    final String entityName, final Entity rootEntity) {

    if (rootEntity.getDatastore() != null && !StringUtil
      .isNullOrEmpty(rootEntity.getDatastore().getSchemaName())) {

      final Entity[] entities;

      final Datastore datastore =
        getDatastoreInstance(rootEntity.getDatastore().getSchemaName());
      final Entity entity = datastore.readEntity(rootEntityID);
      entities = entity.getChildEntities(datastore.getEntityType(entityName));

      return entities;
    } else {
      return getEntities(rootEntityID, entityName);
    }
   ...
   * @throws NoSuchSchemaException
   */
  public static Datastore getDatastoreInstance(final String schemaName) {
  ...
  public static Entity retrieveChildEntity(final Entity parentEntity,
    ...
    // scripts
    if (parentEntity.getDatastore() != null && !StringUtil
      .isNullOrEmpty(parentEntity.getDatastore().getSchemaName())) {
      datastore =
        getDatastoreInstance(parentEntity.getDatastore().getSchemaName());
    } else {

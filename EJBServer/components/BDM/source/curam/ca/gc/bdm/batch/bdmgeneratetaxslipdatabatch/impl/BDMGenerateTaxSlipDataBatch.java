...
import curam.ca.gc.bdm.entity.bdmgeneratepaymentstagingdata.struct.BDMSearchTaxSlipEligClientsKey;
...
public class BDMGenerateTaxSlipDataBatch extends
  curam.ca.gc.bdm.batch.bdmgeneratetaxslipdatabatch.base.BDMGenerateTaxSlipDataBatch {
  ...
  public void process(final BDMGenerateTaxSlipDataBatchKey key)
    ...
    final BDMGenerateTaxSlipDataBatchWrapper chunkWrapper =
      new BDMGenerateTaxSlipDataBatchWrapper(this);

    final BDMGenerateTaxSlipDataBatchStreamWrapper streamWrapper =
      new BDMGenerateTaxSlipDataBatchStreamWrapper(
        new BDMGenerateTaxSlipDataBatchStream(), key);
    ...
    batchStreamHelper.runChunkMain(
      BATCHPROCESSNAME.BDM_GENERATE_TAX_SLIP_DATA_BATCH, key, chunkWrapper,
      batchProcessingIDList, chunkMainParameters, streamWrapper);

  }

  /**
   * retrieves the list of concern roles that need tax slips generated
   *
  ...
  private BatchProcessingIDList getBatchProcessingIDs(final int taxYear)
    ...
    // search for eligible clients for the given tax year
    final BDMSearchTaxSlipEligClientsKey searchEligClientsKey =
      new BDMSearchTaxSlipEligClientsKey();
    searchEligClientsKey.addToTaxSlipInd = true;
    final ConcernRoleKeyList concernRoleList = BDMPaymentInstrumentDAFactory
      .newInstance().searchTaxSlipEligClients(searchEligClientsKey);

    // add them to batch processing ID list
    final BatchProcessingIDList batchProcessingIDList =
      new BatchProcessingIDList();

    for (final ConcernRoleKey concernRoleKey : concernRoleList.dtls) {
      final BatchProcessingID recordToAdd = new BatchProcessingID();
      recordToAdd.recordID = concernRoleKey.concernRoleID;
      batchProcessingIDList.dtls.add(recordToAdd);
    }
  ...
  private ChunkMainParameters getChunkMainParameters() {
    ...
    chunkMainParameters.chunkSize = Configuration.getIntProperty(
      EnvVars.BDM_CURAM_GENERATE_TAX_SLIPS_DATA_BATCH_CHUNK_SIZE);
    chunkMainParameters.dontRunStream = Configuration.getBooleanProperty(
      EnvVars.BDM_CURAM_GENERATE_TAX_SLIPS_DATA_BATCH_DONT_RUN_STREAM);
    chunkMainParameters.startChunkKey =
      Long.parseLong(Configuration.getProperty(
        EnvVars.BDM_CURAM_GENERATE_TAX_SLIPS_DATA_BATCH_START_CHUNK_KEY));
    chunkMainParameters.unProcessedChunkReadWait =
      Configuration.getIntProperty(
        EnvVars.BDM_CURAM_GENERATE_TAX_SLIPS_DATA_BATCH_UNPROCESSED_CHUNK_READ_WAIT);
    chunkMainParameters.processUnProcessedChunks =
      Configuration.getBooleanProperty(
        EnvVars.BDM_CURAM_GENERATE_TAX_SLIPS_DATA_BATCH_PROCESS_UNPROCESSED_CHUNKS);
  ...
  public void sendBatchReport(final String instanceID,
    ...
    curamBatchObj.setEmailSubject(
      BDMGENERATETAXSLIPBATCH.INF_GEN_ORIG_TAX_SLIP_BATCH_SUB);

    // set output file id
    curamBatchObj.outputFileID = BDMGENERATETAXSLIPBATCH.INF_GEN_ORIG_TAX_SLIP

...
<xsl:output method="xml" indent="yes"/>
  ...
  <!--
    Template to write a standalone IncomingAddToCase page (i.e. outside the wizard) for an entity.

    @param entityElement XML Element containing the entity
    @param baseDirectory Base Directory to generate to
  -->
  <xsl:template name="IncomingAddToCaseConstantsProperties">
    ...
    <xsl:call-template name="IncomingAddToCaseConstantsProperties-AllLocales">
  ...
  <xsl:template name="IncomingAddToCaseConstantsProperties-AllLocales">
    ...
    <xsl:if test="$locales">
      ...
      <xsl:choose>
        ...
        <xsl:when test="contains($locales,',')">
          ...
          <xsl:call-template name="IncomingAddToCaseConstantsProperties-OneLocale">
  ...
  <xsl:template name="IncomingAddToCaseConstantsProperties-OneLocale">
    ...
    <redirect:write select="$localizedFileName">
      ...
      <!-- BEGIN, 191675, JAY -->
    <!-- hasAutoEndDate variable to IncomingAddToCase end date wizard pages -->
    ...
    <!-- END, 191675, JAY -->
      ...
      <xsl:if test="$isTopLevel='false'">
        <xsl:variable name="selectAllWizardName"><xsl:call-template name="Utilities-IncomingAddToCaseWizards-SelectAll-getStartingPage">
       ...
       <xsl:if test="$isTopLevel='true' and $hasAutoEndDate='true' and $hasCoreEmployment='false'">
        <xsl:variable name="selectEndDateWizardName"><xsl:call-template name="Utilities-IncomingAddToCaseWizards-WithEndDate-getStartingPage">
          <xsl:with-param name="entityElement" select="$entityElement"/><xsl:with-param name="caseType" select="$caseType"></xsl:with-param>
        </xsl:call-template></xsl:variable>
        <xsl:text>&#xa;</xsl:text>
      ...
      <xsl:if test="$isTopLevel='true' and $hasCoreEmployment='true'">
        <xsl:variable name="selectEmploymentOnlyWizardName"><xsl:call-template name="Utilities-IncomingAddToCaseWizards-SelectEmploymentOnly-getStartingPage">
          <xsl:with-param name="entityElement" select="$entityElement"/><xsl:with-param name="caseType" select="$caseType"></xsl:with-param>
        </xsl:call-template></xsl:variable>
        <xsl:text>&#xa;</xsl:text>
      ...
      <xsl:if test="$hasParent='true'">
        ...
        <xsl:variable name="selectWizardName"><xsl:call-template name="Utilities-IncomingAddToCaseWizards-WithPreSelection-getStartingPage">
